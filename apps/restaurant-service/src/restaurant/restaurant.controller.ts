...
export class RestaurantController {
  ...
  getReviews(@Param("id") id: string) {
    return this.restaurantService.getAllReviews(id);
  }

  @Get(":id/reviews/me")
  getMyReviews(@Req() req: AuthRequest) {
    return this.restaurantService.getMyReviews(req.headers["x-user-id"]);
  }
  ...
  setHours(
    @Req() req: AuthRequest,
    @Param("id") id: string,
    @Body() body: { hours: DayHoursDto[] },
  ) {
    const role = req.headers["x-user-role"];
    if (role !== "restaurant_owner" && role !== "admin") {
      throw new ForbiddenException(
        "Only restaurant owners can set opening hours",
      );
    }
